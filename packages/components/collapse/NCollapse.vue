...
import { rafTimeout } from '../../utils/index'
interface Collapse {
...
<template>
  <div class="n-collapse">
    <div class="n-collapse-item" :class="{ 'u-collapse-item-active': activeJudge(data.key || index) }" v-for="(data, index) in collapseData" :key="index">
      <div class="u-collapse-header" @click="onClick(data.key || index)">
        <svg focusable="false" v-if="showArrow" class="u-arrow" data-icon="right" aria-hidden="true" viewBox="64 64 896 896">
          <path
      ...
      <div class="u-collapse-content" :class="{ 'u-collapse-copyable': copyable }" :style="`height: ${activeJudge(data.key || index) ? collapseHeight[index] : 0}px;`">
        ...
        <n-button size="small" class="u-copy" type="primary" @click="onCopy(index)">{{ copyTxt }}</n-button>
