...
var postTransform = (code, id, customBlocks) => {
  ...
  const { filename } = _pluginvue.parseVueRequest.call(void 0, id);
...
var postTransformMainEntry = (program, id, customBlocks) => {
  for (const node of program.body) {
    if (node.type === "ImportDeclaration" && node.source.value.includes(QUERY_NAMED_TEMPLATE)) {
      const { name: name2 } = _pluginvue.parseVueRequest.call(void 0, node.source.value).query;
      if (!customBlocks[id])
        customBlocks[id] = {};
...
var PrePlugin = _unplugin.createUnplugin.call(void 0, 
  (userOptions = {}) => {
    ...
    return {
      ...
      load(id) {
        var _a;
        const { filename, query } = _pluginvue.parseVueRequest.call(void 0, id);
...
var plugin = _unplugincombine.createCombinePlugin.call(void 0, (userOptions = {}) => {
  return {
    name,
    plugins: [
