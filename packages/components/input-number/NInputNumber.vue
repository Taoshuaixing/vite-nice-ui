<template>
  <div class="n-input-number" tabindex="1" :style="`width: ${width}px;`">
    <div class="n-input-wrap">
      <span class="u-input-prefix" :class="{ mr3: prefix }">
        <slot name="prefix">{{ prefix }}</slot>
      </span>
      <input v-if="keyboard" autocomplete="off" class="u-input-number" @change="onChange" v-model="numValue" @keydown.up.prevent @keydown="onKeyboard" />
...
function onChange(e: any) {
  ...
  if (!Number.isNaN(parseFloat(value))) {
    // Number.isNaN() 判断传递的值是否为NaN，并检测器类型是否为Number
...
.n-input-number {
  ...
  &:focus-within {
    // 激活时样式
    border-color: var(--nice-primary-color);
    box-shadow: 0 0 0 2px var(--nice-primary-color-light);
  }
  .n-input-wrap {
    display: inline-flex;
    vertical-align: top;
